...
class HuishApiClient {
  ...
  Future<HuishCaptchaResult> getCaptcha() async {
    ...
    final resp = await _client.get(url, headers: {
      'User-Agent': 'Dalvik/2.1.0 (Linux; U; Android 16; PHP110 Build/BP2A.250605.015)',
    });
  ...
  Future<HuishApiResponse> sendSmsCode({
    ...
    return HuishApiResponse.fromJson(jsonDecode(resp.body) as Map<String, dynamic>);
  }
  ...
  Future<HuishApiResponse> getDeviceHome(String deviceId, {int apply = 6}) async {
    final resp = await _get('/api/v1/ui/app/dev/home/1?did=$deviceId&apply=$apply');
    return HuishApiResponse.fromJson(jsonDecode(resp.body) as Map<String, dynamic>);
  }

  Future<HuishApiResponse> getDeviceStatus(String deviceId) async {
    final resp = await _get('/api/v1/ui/app/dev/status?did=$deviceId&more=false');
    return HuishApiResponse.fromJson(jsonDecode(resp.body) as Map<String, dynamic>);
  }

  Future<HuishApiResponse> startDevice(String deviceId, {int ptype = 21}) async {
  ...
  Future<HuishApiResponse> favoriteDevice(String deviceId, {bool remove = false}) async {
  ...
  Future<HuishApiResponse> getBillList({int page = 0, int size = 20, int? status}) async {
  ...
  Future<http.Response> _post(String path, {required Map<String, dynamic> data}) async {
    final url = Uri.parse('$_baseUrl$path');
    return _client.post(
      url,
...
class HuishCaptchaResult {
  ...
  HuishCaptchaResult({required this.s, required this.r, required this.imageBytes, required this.contentType});
}
